...
class CohereChatRequest(BaseChatRequest):
    ...
    seed: float | None = Field(
        default=None,
        title="If specified, the retriever will make a best effort to sample tokens deterministically, such that repeated requests with the same seed and parameters should return the same result. However, determinism cannot be totally guaranteed.",
    )
